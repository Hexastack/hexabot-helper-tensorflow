...
import { TENSORFLOW_NLU_HELPER_NAME } from './settings';
import { NlpParseResultType, RasaNlu } from './types';

@Injectable()
export default class TensorflowNluHelper extends BaseNlpHelper<
  typeof TENSORFLOW_NLU_HELPER_NAME
> {
  constructor(
    ...
    super(TENSORFLOW_NLU_HELPER_NAME, settingService, helperService, logger);
  }
  ...
  async filterEntitiesByConfidence(
    ...
    try {
      ...
      this.logger.error(
        'Tensorflow NLU Helper : Unable to parse nlp result to extract best guess!',
        e,
      );
      return {
        entities: [],
  ...
  async predict(
    ...
    try {
      ...
      this.logger.error('Tensorflow NLU Helper : Unable to parse nlp', err);
      throw err;
    }
  }
}
